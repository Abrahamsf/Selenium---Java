...
public class TestApi {
  ...
  public void testApi(String url,String testType) throws Exception {
	  ...
	  if(testType.toLowerCase().equals("positive")){
		    ...
		    driver.findElement(By.id("inputString")).sendKeys(response);
		    driver.findElement(By.id("validateButton")).click();
		    assertEquals("The JSON input is valid according to RFC 4627 (JSON specfication).", driver.findElement(By.xpath("//div[2]/p")).getText());
	  }
	  else if(testType.toLowerCase().equals("negative")){
		  
		  //if negative response should contain error
